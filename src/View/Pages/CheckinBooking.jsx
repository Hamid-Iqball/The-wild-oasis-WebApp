...
function CheckinBooking() {
  ...
  return (
    ...
    <label htmlFor="" className='font-semibold text-[#5d5f63]'>I confirm that {guests.fullName} has paid the total amount of {formatCurrency(totalPrice)} </label>
    </div>
    
    <div className='flex justify-end gap-2 items-center'>
    <button className='py-2 px-3 text-white bg-orange-700 rounded-md  font-semibold text-center hover:bg-orange-800 hover:duration-300 hover:ease-in-out' disabled={!confirmedPaid} onClick={checkIn} >Check in booking # {bookingId}</button>
